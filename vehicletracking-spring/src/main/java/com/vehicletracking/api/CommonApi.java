...
public class CommonApi {
	...
	public Response getDriversList() {
		...
		char type = 'D'; // setting statically because we need to get all
							// Drivers list in user association
	...
	public Response getCustomersList() {
		userAssosiations = new ArrayList<UserAssosiation>();
		char type = 'C'; // setting statically because we need to get all
							// Drivers list in user association
		userAssosiations = commonApiDAO.getUserAssosiationListByType(type);
		return Response.status(200).entity(userAssosiations).build();
	}
	...
	/**
	 * @method createAssosiation -Here used to create/add the assosiation
	 *         details
	 ...
	 * @Produces(MediaType.APPLICATION_JSON)
	 * 
	 * @Path("/addDriverPhone") public Response createAssosiation(
	 * 
	 ...
	 * User vehicleOwnerUser = null; UserAssosiation userAssosiation = null;
	 * UserAssosiation savedUserAssosiation = null; User driverUser = null; try{
	 * if(!StringUtils.isEmpty(vehicleOwnerPhoneNumber)){ vehicleOwnerUser = new
	 * User(); userAssosiation = new UserAssosiation(); vehicleOwnerUser =
	 * userDAO.getUserByPhone(vehicleOwnerPhoneNumber);
	 * logger.info("Vehicle Owner User is :"+vehicleOwnerUser.getName());
	 * if(!StringUtils
	 * .isEmpty(vehicleOwnerUser)){userAssosiation.setParent_user_master
	 * (vehicleOwnerUser);}
	 * 
	 * } if(!StringUtils.isEmpty(driverPhoneNumber)){ driverUser = new User();
	 * driverUser = userDAO.getUserByPhone(driverPhoneNumber);
	 * 
	 * if(!StringUtils.isEmpty(driverUser)){
	 * logger.info("Got Driver obj "+driverUser
	 * .getPhone_number()+" : "+driverUser.getName());
	 * userAssosiation.setApp_user_master(driverUser); }else{ User driver = new
	 * User(); driver.setPhone_number(driverPhoneNumber);
	 * driver.setIs_active('N'); driver.setApp_download_status('N'); User
	 ...
	 * userAssosiation.setType('D'); } savedUserAssosiation =
	 * commonApiDAO.saveUserAssosiation(userAssosiation); }catch(Exception
	 * e){e.printStackTrace();} return
	 * Response.status(200).entity(savedUserAssosiation).build(); }
	 */
